...
$db = mysqli_connect('localhost', 'root', 'root', 'DoctorsRooms') or die("DB connection failed, contact your admin");
    ...
    //finding the most recently added id

    $check_id="SELECT * FROM Appointments ORDER BY id DESC";
    $check_result=mysqli_query($db, $check_id);
    $user1 = mysqli_fetch_assoc($check_result);

    $send_id=$user1['id'];
    $dateAndTime=$user1['date'] . " at " . $user1['time'];
    $doctor=$user1['doctor'];
    $patient=$user1['patient'];

    // //getting appointment details
    // $appointment_details="SELECT * FROM Appointments WHERE id = '$send_id'";
    // $appointment_result=mysqli_query($db, $appointment_details);
    // $appointment = mysqli_fetch_assoc($appointment_result);

    // $dateAndTime=$appointment['date'] . " | " . $appointment['time'];
    ...
    <body>
     ...
     <?php endif ?>
        ...
        <div class="popup">
            ...
            <h3>Appointment has been created</h3>
            <p style="margin-top:15px">The appointment has been set for <?php echo $dateAndTime?></p>
            <p style="margin-top:170px; font-size:13px;">Appointment with <?php echo $doctor?> and <?php echo $patient?></p>
            <a href="https://mail.google.com/mail/u/0/#inbox?compose=new"> <div class="sendConfirmation">Send <?php echo $patient?> confirmation</div></a>
